...
contract InsuranceFundraising {
    ...
    // 🔒 Fixed bond price: 0.5 ETH per bond
    ...
    constructor(
        ...
        uint _yieldRateBasisPoints,     // yield per quarter in basis points
    ...
    function buyBonds(uint _amount) external payable fundraisingActive {
        ...
        uint totalCost = _amount * bondPrice;  // 0.5 ETH * amount
    ...
    function payQuarterlyCoupon() external payable onlyOwner {
        ...
        if (beforeBalance > 0 && yieldRate > 0) {
            uint256 yieldGrowth = (beforeBalance * yieldRate) / 10000;
            if (yieldGrowth > 0) {
        ...
        if (totalCouponsPaid >= TOTAL_COUPONS) {
    matureContract();
        }
    }
    ...
    uint256 beforeBalance = yieldToken.balanceOf(address(this));
    uint256 yieldGrowth = (beforeBalance * yieldRate) / 10000;
    uint256 newBalance = beforeBalance + yieldGrowth;
    uint256 premium = (newBalance * riskPremiumRate) / 10000;
    return premium;
    }
